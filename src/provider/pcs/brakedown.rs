...
pub struct MultilinearBrakedownParam<E: Engine> {
  num_vars: usize,
  num_rows: usize,
  brakedown: Brakedown<E::Scalar>,
}

impl<E: Engine> MultilinearBrakedownParam<E> {
  ...
  pub fn brakedown(&self) -> &Brakedown<E::Scalar> {
    &self.brakedown
  }
}
...
// #[serde(bound(serialize = "E::Scalar: Serialize", deserialize = "E::Scalar: DeserializeOwned"))]
...
impl<E: Engine, H: Hash> MultilinearBrakedownCommitment<E, H> {
  fn from_root(root: Output<H>) -> Self {
    Self { root, rows: vec![] }
  }

  pub fn rows(&self) -> &[E::Scalar] {
    &self.rows
  }
...
// impl<E, H: Hash> AsRef<[Output<H>]> for MultilinearBrakedownCommitment<E, H> {
    ...
    S: BrakedownSpec,
{
    ...
    fn setup(
        ...
        let gens = E::GE::from_label(label, width + 1);
        let num_vars = n.ilog2() as usize;
        let brakedown = Brakedown::new_multilinear::<S>(num_vars, 20.min((1 << num_vars) - 1), gens);
        ...
        println!(
            "setup: num_vars = {:?}, num_rows = {:?}",
            param.num_vars, param.num_rows
        );
        Ok((param,param))
    }
